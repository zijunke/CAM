...
# Specify the CFA model for extroversion
#-------------------------------------
myM <- '
...
sel.names = c('L1','L2')
#sel.names = c('L5','L6')
mu.L = coef(fit)[sel.names] # factor loadings estimates
V.L = vcov(fit)[sel.names,sel.names] # their precisions
```
...
t1 = data.frame(ind.L,gphi,rri,ni=dall$N)
# Export relative information for retrospective analysis
# i.e., Table 1 and the estimated L and Cov(L)
write.xlsx(t1,'t1.xlsx', col.names=T, row.names=F)
write.xlsx(cbind(mu.L,V.L),'t1.xlsx','Sheet2',col.names=T,row.names=F,append=T)
t1 # Table 1
```
