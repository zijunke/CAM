...
Nrr = 500
Frho.list = list(v=1,sdv = 0,M.ind = matrix(1,1,1))
L.list = list(v=rep(NA,6),sdv = rep(NA,6),
	M.ind = matrix(1:6,6,1,byrow = T))
Phi.list = list(v = c(0.25,0.14,0.03,0.23,0.17,0.21,0.04,0.03,0.06),
	sdv = rep(0.05,9),
	M.ind = matrix(c(11,1,2,7,10,10,1,12,3,10,8,10,2,3,13,10,10,9,
	  7,10,10,14,4,5,10,8,10,4,15,6,10,10,9,5,6,16),6,6,byrow = T))
...
'
CFAM.SWB <- '
	SWB =~ L1*V1 + L4*V4 + L2*V2 + L5*V5 + L3*V3 + L6*V6
	V1 ~~ V2; V1~~V3; V2 ~~ V3
	V4 ~~ V5; V4~~V6; V5 ~~ V6
	V1 ~~ V4;V2 ~~ V5; V3 ~~ V6
	V1 ~~ VE1*V1; V4 ~~ VE1*V4
...
for(rri in 1:2){#1:2
   ...
   rr.model$SWB$L$v[1:6] = L$v[3:8,rri]
   rr.model$SWB$L$sdv[1:6] = rep(L$sdv[sdLi],6)
